...
def create_movement_maps(map,nCols,nRows):
    ...
    for indx,point in enumerate(map):
        ...
        left_pathway  = False if ( (left_neighbor is None)  or (ord(map[left_neighbor])-ord(point)>1)) else True
        up_pathway    = False if ( (up_neighbor is None)    or (ord(map[up_neighbor])-ord(point)>1)) else True
        right_pathway = False if ( (right_neighbor is None) or (ord(map[right_neighbor])-ord(point)>1)) else True
        down_pathway  = False if ( (down_neighbor is None)  or (ord(map[down_neighbor])-ord(point)>1)) else True

        allowed_pathway.append([left_pathway, up_pathway, right_pathway, down_pathway])

    return neighbors,allowed_pathway

if __name__ == "__main__":
    file = sys.argv[1]

    map,nCols,nRows,Sindx,Eindx  = read_map(file)
    neighbors,pathways = create_movement_maps(map,nCols,nRows)
