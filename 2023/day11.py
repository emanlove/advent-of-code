...
def read_skydata(filename):
    ...
    for rindx,row in enumerate(lines):
        for cindx,col in enumerate(row):
            if col == '#':
                ...
                r = rindx + shift_by(empty_rows,rindx)
                c = cindx + shift_by(empty_cols,cindx)
    ...
    print(f"The sum of the lengths of the shortest path between every pair of galaxies is {sum(distances)}")

def shift_by(empties, indx):
    return sum([1 for e in empties if indx > e])
